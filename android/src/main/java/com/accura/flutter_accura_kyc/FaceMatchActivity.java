...
//Activity for FaceMatch & liveness
        ...
        if (bundle.containsKey("with_face")) {
            ...
            if (witFace) {
                ...
                if (base64Data.length() > 0) {
                    Bitmap face = FlutterAccuraKycPlugin.getBase64ToBitmap(base64Data);
                    face1 = face;
                    faceHelper.setInputImage(face);
                }
                ...
                if (!isLiveness) {
                    if (!bundle.containsKey("face1")) {
                        FlutterAccuraKycPlugin.faceCL.error("101","Missing face1 configuration", null);
                        this.finish();
                        return;
                    }
                    if (bundle.containsKey("face2")) {
                        ...
                        if (isFace2) {
                            if (FlutterAccuraKycPlugin.face1 == null) {
                              FlutterAccuraKycPlugin.faceCL.error("101","Please first take Face1 photo", null);
                                this.finish();
                                return;
                            } else {
                                face1 = FlutterAccuraKycPlugin.face1;
                                faceHelper.setInputImage(face1);
                            }
                        }
                    } else {
                      FlutterAccuraKycPlugin.faceCL.error("101","Missing face2 configuration", null);
                        this.finish();
                        return;
                    }
                }
          ...
          FlutterAccuraKycPlugin.faceCL.error("101","Missing with_face configuration", null);
            this.finish();
            return;
        }
        try {
...
//        }
//        if (livenessCustomization.isRecordVideo && FlutterAccuraKycPlugin.isLivenessGetVideo) {
//            File vid = new File(FlutterAccuraKycPlugin.livenessVideo);
//            if (vid.exists()) {
...
//        cameraScreenCustomization.setGlarePercentage(bundle.getInt("setGlarePercentage_0", getResources().getInteger(R("setGlarePercentage_0", "integer"))), bundle.getInt("setGlarePercentage_1", getResources().getInteger(R("setGlarePercentage_1", "integer"))));
    ...
    public void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == 202) {
            if (data == null) {
              FlutterAccuraKycPlugin.faceCL.error("101","No data", null);
                this.finish();
                return;
            }
                ...
                if (result.getStatus().equals("1")) {
                    livenessResult = handleVerificationSuccessResult(result, results);
                     if (witFace) {
                       ...
                       FlutterAccuraKycPlugin.faceCL.success(livenessResult.toString());
                         this.finish();
                     }

                } else {
                  FlutterAccuraKycPlugin.faceCL.error("101", result.getErrorMessage(), null);
                    if (result.getVideoPath() != null) {
                      FlutterAccuraKycPlugin.isLivenessGetVideo = true;
                      FlutterAccuraKycPlugin.livenessVideo = result.getVideoPath().getPath();
            ...
            if (result.getLivenessResult() != null) {
                if (result.getLivenessResult().getLivenessStatus()) {
                    try {
                        ...
                        if (result.getFaceBiometrics() != null) {
                            caResults.put("detect", FlutterAccuraKycPlugin.getImageUri(result.getFaceBiometrics(), "live_detect", getFilesDir().getAbsolutePath()));
                        }
    ...
    public void handleVerificationSuccessResultFM(final AccuraVerificationResult result) {
        if (result != null) {
            if (face1 == null) {
                if (result.getFaceBiometrics() != null) {
                    FlutterAccuraKycPlugin.face1 = face1 = result.getFaceBiometrics();
                    JSONObject results = new JSONObject();
                    try {
                        results.put("status", false);
                        ...
                        if (detectFace1 == null) {
                            results.put("img_1", FlutterAccuraKycPlugin.getImageUri(FlutterAccuraKycPlugin.face1, "img_1", fileDir));
                        } else {
                            results.put("img_1", FlutterAccuraKycPlugin.getImageUri(detectFace1, "img_1", fileDir));
                        }
                  ...
                  FlutterAccuraKycPlugin.faceCL.success(results.toString());
                    this.finish();
                    return;
                }
                return;
        ...
        if (face2 != null) {
            ...
            try {
                if (!isLiveness) {
                    ...
                    if (!witFace) {
                        results.put("img_1", FlutterAccuraKycPlugin.getImageUri(detectFace1, "img_1", fileDir));
                        results.put("img_2", FlutterAccuraKycPlugin.getImageUri(detectFace2, "img_2", fileDir));
                    } else {
                        results.put("detect", FlutterAccuraKycPlugin.getImageUri(detectFace2, "img_1", fileDir));
                    }
              ...
              FlutterAccuraKycPlugin.faceCL.error("101","Error found in data. Please try again", null);
                e.printStackTrace();
                this.finish();
            }
            this.finish();
