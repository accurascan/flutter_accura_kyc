...
public class SwiftFlutterAccuraKycPlugin: NSObject, FlutterPlugin {
  
    var goNativeCallBack: FlutterResult? = nil
    var goNativeAction: String = ""
    var goNativeArgs: NSArray = []
    var viewController: FlutterViewController? = nil;
    var viewControllerWindow: UIWindow? = nil;
    var defaultAppOriantation: String = "portrait";
    
    public static func register(with registrar: FlutterPluginRegistrar) {
        let channel = FlutterMethodChannel(name: "flutter_accura_kyc", binaryMessenger: registrar.messenger())
        let instance = SwiftFlutterAccuraKycPlugin()
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        if (call.arguments != nil) {
            let argsNew = call.arguments as! NSDictionary
            let jsonString = argsNew["arguments"] as? String
            let data: Data? = jsonString?.data(using: .utf8)
            args = ((try? JSONSerialization.jsonObject(with: data!, options: [])) as? NSArray)!
        }
        ...
        if (call.method == "getPlatformVersion") {
            ...
            getMetaData(result)
        } else if (call.method == "setupAccuraConfig") {
            
            setupAccuraConfig(args, callback: result)
        } else if (call.method == "startOcrWithCard") {
            
            startOcrWithCard(args, callback: result)
        } else if (call.method == "startMRZ") {
            
            startMRZ(args, callback: result)
        } else if (call.method == "startBankCard") {
            
            startBankCard(args, callback: result)
        } else if (call.method == "startBarcode") {
            
            startBarcode(args, callback: result)
        } else if (call.method == "startFaceMatch") {
            
            startFaceMatch(args, callback: result)
        } else if (call.method == "startLiveness") {
            
            startLiveness(args, callback: result)
        }
    }
    
    func cleanFM(callBack: FlutterResult) {
    ...
    func getMetaData(_ callback: @escaping FlutterResult) {
        ...
        DispatchQueue.main.async {
            self.viewController = FlutterViewController()
            self.viewControllerWindow = FlutterViewController().view.window
            ...
            self.goNativeCallBack!(SwiftFlutterAccuraKycPlugin.convertJSONString(results: results))
        }
    }
...
//        ScanConfigs.accuraConfigs["app_orientation"] = self.goNativeArgs[2] as! String
        ...
        DispatchQueue.main.async {
            ...
            DispatchQueue.main.async {
                let nav = UINavigationController(rootViewController: LVController)
                nav.isNavigationBarHidden = true
                nav.modalPresentationStyle = .fullScreen
                nav.modalTransitionStyle = .coverVertical
                getTopMostViewController()?.present(nav, animated: true, completion: {
