...
//View controller for document scanning.
...
//        self.win!.rootViewController = reactViewController!
        ...
        if (!isCloseMe) {
            callBack!(FlutterError.init(code: "101", message: "User decline scanning of document", details: nil))
        }
    }
...
//     }
    ...
    func dlPlateNumber(_ plateNumber: String!, andImageNumberPlate imageNumberPlate: UIImage!) {
        ...
        if let frontUri = SwiftFlutterAccuraKycPlugin.getImageUri(img: imageNumberPlate, name: nil) {
...
//            resultmodel.arrayocrBackSideDataValue = ocrBackModeResult!.arrayocrBackSideDataValue
        ...
        if let faceUri = SwiftFlutterAccuraKycPlugin.getImageUri(img: resultmodel.faceImage, name: nil) {
            results["face"] = faceUri
        }
        if let frontUri = SwiftFlutterAccuraKycPlugin.getImageUri(img: resultmodel.frontSideImage, name: nil) {
            results["front_img"] = frontUri
        }
        if let backUri = SwiftFlutterAccuraKycPlugin.getImageUri(img: resultmodel.backSideImage, name: nil) {
            ...
            if let k = data.key as? String {
                if k == "Signature" {
                    if let base64img = data.value as? String {
                        ...
                        if let sigUri = SwiftFlutterAccuraKycPlugin.getImageUri(img: decodedimage, name: nil) {
    ...
    func isBothSideAvailable(_ isBothAvailable: Bool) {
        ...
        if !isBothAvailable && isNeedBackSideFirst(){
            ...
            callBack!(FlutterError.init(code: "101", message: "Back Side not available", details: nil))
            closeMe()
        }
    }
    ...
    func recognizeSucceedBarcode(_ message: String!, back BackSideImage: UIImage!, frontImage FrontImage: UIImage!, face FaceImage: UIImage!) {
        ...
        if(!isBarcodeEnabled) {
            ...
            if cardType == 1 {
                if FaceImage != nil{
                    if let frontUri = SwiftFlutterAccuraKycPlugin.getImageUri(img: FaceImage!, name: nil) {
                        results["face"] = frontUri
                    }
                }
                if FrontImage != nil{
                    if let frontUri = SwiftFlutterAccuraKycPlugin.getImageUri(img: FrontImage!, name: nil) {
                        results["front_img"] = frontUri
                    }
                }
                if BackSideImage != nil{
                    if let frontUri = SwiftFlutterAccuraKycPlugin.getImageUri(img: BackSideImage!, name: nil) {
    ...
    func recognizeFailed(_ message: String!) {
        ...
        callBack!(FlutterError.init(code: "101", message: message, details: nil))
        closeMe()
    }
    ...
    func sendRecognizResults(face: UIImage?, front: UIImage?, back: UIImage?) {
        ...
        if let recogFrontImg = face{
            if let frontUri = SwiftFlutterAccuraKycPlugin.getImageUri(img: recogFrontImg, name: nil) {
                results["face"] = frontUri
            }
        }
        ...
        if let recogBackImg = back{
            if let frontUri = SwiftFlutterAccuraKycPlugin.getImageUri(img: recogBackImg, name: nil) {
                results["back_img"] = frontUri
            }
        }
        results["front_data"] = frontData
    ...
    func recognizSuccessBankCard(_ cardDetail: NSMutableDictionary!, andBankCardImage bankCardImage: UIImage!) {
        ...
        if let frontUri = SwiftFlutterAccuraKycPlugin.getImageUri(img: bankCardImage, name: nil) {
            results["front_img"] = frontUri
        }
