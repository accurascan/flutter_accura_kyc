        ...
        if !isLivenessDone {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                if (!self.isCalledCallBack) {
                    ...
                    self.callBack!(FlutterError.init(code: "101", message: "User decline liveness", details: nil))
                }
            }
        }
    }
    ...
    func livenessData(_ stLivenessValue: String!, livenessImage: UIImage!, status: Bool, imagePath: String!) {
        ...
        if status == true {
            ...
            if gl.face2Detect != nil {
                results["detect"] = SwiftFlutterAccuraKycPlugin.getImageUri(img: SwiftFlutterAccuraKycPlugin.resizeImage(image: livenessImage, targetSize: gl.face2Detect!.bound), name: nil)
            } else {
                results["detect"] = SwiftFlutterAccuraKycPlugin.getImageUri(img: livenessImage, name: nil)
            }
            if imagePath != "" {
